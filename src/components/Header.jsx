    ...
    <header className="header">
      <div className="header-inner">
        ...
        <nav className={`nav ${mobileMenuOpen ? 'mobile-open' : ''}`}>
          ...
          <span className="nav-link" onClick={() => handleNavigate('home')}>{t('home')}</span>
          <span className="nav-link" onClick={() => handleNavigate('catalog')}>{t('ourCakes')}</span>
          <span className="nav-link" onClick={() => handleNavigate('contact')}>{t('contact')}</span>
          {isAdmin && <span className="nav-link" onClick={() => handleNavigate('admin')} style={{color:'var(--terracotta)',fontWeight:600}}>{t('admin')}</span>}
          
          <div className="mobile-nav-actions">
            {user ? (
              <span className="nav-link" onClick={() => { logout(); onNavigate('home'); setMobileMenuOpen(false); }}>
                <LogOut size={18} style={{marginRight:'8px', verticalAlign:'text-bottom'}}/>
                {t('logout')}
              </span>
            ) : (
              <span className="nav-link" onClick={() => { setShowLogin(true); setMobileMenuOpen(false); }}>
                {t('login')}
              </span>
            )}
          </div>
        ...
        <div className="nav-actions">
          ...
          <div className="icon-btn-group">
            ...
            <button className="cart-btn" onClick={() => onNavigate('settings')} title={t('settings')}>
              <Settings size={20} />
            </button>
      ...
      <SearchModal isOpen={showSearch} onClose={() => setShowSearch(false)} products={products || []} formatCurrency={formatCurrency} />
    </>
  );
};
