      ...
      <div className="form-card" style={{marginBottom:'2rem'}}>
        ...
        <form onSubmit={handleSave}>
          <div className="form-row">
            <div className="form-group"><label>{t('ingredientName')} *</label><input type="text" value={editing?.name || newIng.name} onChange={e => editing ? setEditing({...editing, name: e.target.value}) : setNewIng({...newIng, name: e.target.value})} required onInvalid={e => e.target.setCustomValidity(t('fieldRequired'))} onInput={e => e.target.setCustomValidity('')} /></div>
            <div className="form-group"><label>{t('unit')} *</label><input type="text" value={editing?.unit || newIng.unit} onChange={e => editing ? setEditing({...editing, unit: e.target.value}) : setNewIng({...newIng, unit: e.target.value})} placeholder="lb, oz, qt, etc." required onInvalid={e => e.target.setCustomValidity(t('fieldRequired'))} onInput={e => e.target.setCustomValidity('')} /></div>
            <div className="form-group"><label>{t('pricePerUnit')} ({symbol}) *</label><input type="number" step="0.01" min="0" value={editing?.price || newIng.price} onChange={e => editing ? setEditing({...editing, price: e.target.value}) : setNewIng({...newIng, price: e.target.value})} required onInvalid={e => e.target.setCustomValidity(t('fieldRequired'))} onInput={e => e.target.setCustomValidity('')} /></div>
          </div>
          <button type="submit" className="form-submit" style={{width:'auto',padding:'0.8rem 2rem',marginTop:0}}>{editing ? t('update') : t('add')} {t('ingredients')}</button>
          ...
          <table className="admin-table">
            <thead>
              <tr>
                ...
                <th style={{width:'20%'}}>{t('pricePerUnit')} ({symbol})</th>
                  ...
                  <td>
                    {isEditing ? (
                      <input 
                        ...
                        autoFocus
                        required
                        onInvalid={e => e.target.setCustomValidity(t('fieldRequired'))}
                        onInput={e => e.target.setCustomValidity('')}
                      />
                    ) : (
                      <strong style={{color:'var(--espresso)', fontSize:'0.95rem'}}>{ing.name}</strong>
                    )}
                  </td>
                  <td>
                    {isEditing ? (
                      ...
                      <span style={{color:'var(--chocolate)'}}>{ing.unit}</span>
                    )}
                  </td>
                  <td>
                    {isEditing ? (
                      ...
                      <span>{symbol} {(ing.price * rate).toFixed(2)}</span>
                    )}
                  </td>
                  <td>
                    {isUsed ? (
                      <span className="status used" title={`Used in: ${usedIn.map(p => p.name).join(', ')}`}>{t('used')} ({usedIn.length})</span>
                    ) : (
