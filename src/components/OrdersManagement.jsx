              ...
              <div key={order.id} style={{padding:'1.5rem',background:'var(--blush)',borderRadius:'16px',border:'1px solid rgba(0,0,0,0.1)'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',marginBottom:'1rem',flexWrap:'wrap',gap:'1rem'}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:'0.9rem',color:'var(--chocolate)',marginTop:'0.5rem'}}>
                      <strong>{t('address')}:</strong> {order.address}
                    </div>
                     <div style={{fontSize:'0.9rem',color:'var(--chocolate)',marginTop:'0.5rem'}}>
                      <strong>{t('delivery')}:</strong> {order.date} @ {order.time} ({t(order.timeOfDay)})
                    </div>
                  </div>
                  <div style={{textAlign:'right'}}>
                      <div style={{fontWeight:'600',color:'var(--terracotta)',fontSize:'1.2rem'}}>{formatPrice(order.total)}</div>
                      <div style={{fontSize:'0.9rem',marginTop:'0.5rem'}}>
                          <div><strong>{t('items')}:</strong></div>
                          {order.items.map(i => (
                              <div key={`${order.id}-${i.id}`} style={{fontSize:'0.85rem',opacity:0.8}}>{i.name} x{i.quantity}</div>
                          ))}
                      </div>
                  </div>
                </div>
